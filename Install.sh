sudo pacman -S --needed git base-devel && git clone https://aur.archlinux.org/yay.git && cd yay && makepkg -si && yay -S --needed --noconfirm gvfs thunar-archive-plugin thunar-media-tags-plugin thunar-volman \
	java-environment-common java-runtime-common jdk17-openjdk jq jre-openjdk jre8-openjdk jre8-openjdk-headless \
        ...
        libxinerama lib33-libxinerama ncurses lib32-ncurses opencl-icd-loader lib32-opencl-icd-loader libxslt lib32-libxslt libva lib32-libva gtk3 lib32-gtk3 gst-plugins-base-libs lib32-gst-plugins-base-libs \
        vulkan-icd-loader lib33-vulkan-icd-loader obs-studio discord mangohud lib32-mangohud goverlay gamescope bluez solaar bluez-utils lib32-libpulse pipewire pipewire-pulse pipewire-alsa linux-headers xwaylandvideobridge \
 	nvidia nvidia-utils lib32-nvidia-utils nvidia-settings opencl-nvidia \
       	pamac-all networkmanager greetd nwg-hello egl-wayland && sudo usermod -aG gamemode izaiaha && sudo systemctl enable fstrim.timer && sudo systemctl enable NetworkManager && sudo systemctl enable cronie.service && sudo systemctl enable greetd.service && echo /usr/bin/fish | sudo tee -a /etc/shells && chsh -s /usr/bin/fish)
