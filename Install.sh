sudo pacman -S --needed git base-devel && git clone https://aur.archlinux.org/paru.git && cd paru && makepkg -si && cd ~ && paru -S --needed --noconfirm gvfs thunar-archive-plugin thunar-media-tags-plugin thunar-volman \
	nvidia nvidia-utils lib32-nvidia-utils nvidia-settings opencl-nvidia \
 	java-environment-common java-runtime-common jdk17-openjdk jq jre-openjdk jre8-openjdk jre8-openjdk-headless \
        adwsteamgtk appimagelauncher protonplus polkit-gnome gnome-disk-utility snapper btrfs-assistant fzf mpv fastanime fish fisher firefox nvim cron \
        ...
        vulkan-icd-loader lib32-vulkan-icd-loader obs-studio discord mangohud lib32-mangohud goverlay gamescope bluez solaar bluez-utils lib32-libpulse pipewire pipewire-pulse pipewire-alsa linux-headers xwaylandvideobridge \
       	pamac-all networkmanager egl-wayland && sudo usermod -aG gamemode izaiaha && sudo systemctl enable fstrim.timer && sudo systemctl enable NetworkManager && sudo systemctl enable cronie.service && echo /usr/bin/fish | sudo tee -a /etc/shells && chsh -s /usr/bin/fish)
